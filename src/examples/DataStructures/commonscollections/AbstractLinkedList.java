 ...
 */
package DataStructures.commonscollections;
...
/**
 * An abstract implementation of a linked list which provides numerous points for subclasses to
 * override.
 * <p>
 * Overridable methods are provided to change the storage node and to change how nodes are added to
 * and removed. Hopefully, all you need for unusual subclasses is here.
 *
 * @since 3.0
 */
public abstract class AbstractLinkedList<E> implements List<E> {
  ...
  /*
   * Implementation notes: - a standard circular doubly-linked list - a marker node is stored to
   * mark the start and the end of the list - node creation and removal always occurs through
   * createNode() and removeNode(). - a modification count is kept, with the same semantics as
   * {@link java.util.LinkedList}. - respects {@link AbstractList#modCount}
   */

  /**
   * A {@link Node} which indicates the start and end of the list and does not hold a value. The
   * value of <code>next</code> is the first item in the list. The value of of <code>previous</code>
  ...
  /**
   * The equivalent of a default constructor, broken out so it can be called by any constructor and
   * by <code>readObject</code>. Subclasses which override this method should make sure they call
   * super, so the list is initialised properly.
   */
  protected void init() {
   ...
   * {@inheritDoc}
   * <p>
   * This implementation iterates over the elements of this list, checking each element in turn to
   * see if it's contained in <code>coll</code>. If it's contained, it's removed from this list. As
   * a consequence, it is advised to use a collection type for <code>coll</code> that provides a
   * fast (e.g. O(1)) implementation of {@link Collection#contains(Object)}.
   */
  @Override
   ...
   * <p>
   * This implementation iterates over the elements of this list, checking each element in turn to
   * see if it's contained in <code>coll</code>. If it's not contained, it's removed from this list.
   * As a consequence, it is advised to use a collection type for <code>coll</code> that provides a
   * fast (e.g. O(1)) implementation of {@link Collection#contains(Object)}.
   */
  @Override
  ...
  // -----------------------------------------------------------------------
  /**
   * Compares two values for equals. This implementation uses the equals method. Subclasses can
   * override this to match differently.
   *
   * @param value1
  ...
  /**
   * Updates the node with a new value. This implementation sets the value on the node. Subclasses
   * can override this to record the change.
   *
   * @param node
   *          node to update
   * @param value
  ...
  /**
   * Creates a new node with previous, next and element all set to null. This implementation creates
   * a new empty Node. Subclasses can override this to create a different class.
   *
  ...
  /**
   * Creates a new node with the specified properties. This implementation creates a new Node with
   * data. Subclasses can override this to create a different class.
   *
   * @param value
  ...
  /**
   * Creates a new node with the specified object as its <code>value</code> and inserts it before
   * <code>node</code>.
   * <p>
  ...
  /**
   * Creates a new node with the specified object as its <code>value</code> and inserts it after
   * <code>node</code>.
   * <p>
   * This implementation uses {@link #createNode(Object)} and
   * {@link #addNode(AbstractLinkedList.Node,AbstractLinkedList.Node)}.
   ...
   * @param endMarkerAllowed
   *          whether or not the end marker can be returned if startIndex is set to the list's size
   * @return the node at the given index
   * @throws IndexOutOfBoundsException
   *           if the index is less than 0; equal to the size of the list and endMakerAllowed is
   *           false; or greater than the size of the list
   */
  protected Node<E> getNode(final int index, final boolean endMarkerAllowed)
    ...
    if (index < 0) {
      throw new IndexOutOfBoundsException(
          "Couldn't get the node: " + "index (" + index + ") less than zero.");
    }
    if (!endMarkerAllowed && index == size) {
      throw new IndexOutOfBoundsException(
          "Couldn't get the node: " + "index (" + index + ") is the size of the list.");
    }
    if (index > size) {
      throw new IndexOutOfBoundsException("Couldn't get the node: " + "index (" + index
          + ") greater than the size of the " + "list (" + size + ").");
    }
   ...
   */
  protected ListIterator<E> createSubListListIterator(final LinkedSubList<E> subList,
      final int fromIndex) {
   ...
   * <p>
   * The first serializable subclass must call this method from <code>writeObject</code>.
   *
   * @param outputStream
   ...
   * <p>
   * The first serializable subclass must call this method from <code>readObject</code>.
   *
   * @param inputStream
   *          the stream to read the object from
   * @throws IOException
   *           if any error occurs while reading from the stream
   * @throws ClassNotFoundException
   *           if a class read from the stream can not be loaded
   */
  @SuppressWarnings("unchecked")
  protected void doReadObject(final ObjectInputStream inputStream)
   ...
   * <p>
   * From Commons Collections 3.1, all access to the <code>value</code> property is via the methods
   * on this class.
   */
  ...
  protected static class LinkedListIterator<E> implements ListIterator<E>, OrderedIterator<E> {
    ...
    /**
     * The last node that was returned by {@link #next()} or {@link #previous()}. Set to
     * <code>null</code> if {@link #next()} or {@link #previous()} haven't been called, or if the
     * node has been removed with {@link #remove()} or a new node added with {@link #add(Object)}.
     * Should be accessed through {@link #getLastNodeReturned()} to enforce this behaviour.
     */
    protected Node<E> current;

    /**
     * The modification count that the list is expected to have. If the list doesn't have this
     * count, then a {@link java.util.ConcurrentModificationException} may be thrown by the
     * operations.
     */
     ...
     *           If {@link #next()} or {@link #previous()} haven't been called, or if the node has
     *           been removed with {@link #remove()} or a new node added with {@link #add(Object)}.
     */
  ...
  protected static class LinkedSubList<E> extends AbstractList<E> {
    ...
    protected LinkedSubList(final AbstractLinkedList<E> parent, final int fromIndex,
        final int toIndex) {
      if (fromIndex < 0) {
