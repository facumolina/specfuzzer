...
/**
 * A <code>List</code> implementation that is optimised for fast insertions and removals at any
 * index in the list.
 * <p>
 * This list implementation utilises a tree structure internally to ensure that all insertions and
 * removals are O(log n). This provides much faster performance than both an <code>ArrayList</code>
 * and a <code>LinkedList</code> where elements are inserted and removed repeatedly from anywhere in
 * the list.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * <code>ArrayList</code> is a good general purpose list implementation. It is faster than
 * <code>TreeList</code> for most operations except inserting and removing in the middle of the
 * list. <code>ArrayList</code> also uses less memory as <code>TreeList</code> uses one object per
 * entry.
 * </p>
 * <p>
 * <code>LinkedList</code> is rarely a good choice of implementation. <code>TreeList</code> is
 * almost always a good replacement for it, although it does use slightly more memory.
 * </p>
 *
 * @since 3.1
 */
public class TreeList<E> extends AbstractList<E> {
  ...
  // Added for daikon compatibility
  @Override
   ...
   * <p>
   * This method runs in O(n + log m) time, where m is the size of this list and n is the size of
   * {@code c}.
   *
   * @param c
   ...
   * <p>
   * This node contains the real work. TreeList is just there to implement {@link java.util.List}.
   * The nodes don't know the index of the object they are holding. They do know however their
   * position relative to their parent node. This allows to calculate the index of a node while
   * traversing the tree.
   * <p>
   * The Faedelung calculation stores a flag for both the left and right child to indicate if they
   * are a child (false) or a link as in linked list (true).
   */
  public static class AVLNode<E> {
     ...
     */
    private AVLNode(final int relativePosition, final E obj, final AVLNode<E> rightFollower,
     ...
     * Constructs a new AVL tree from a collection.
     * <p>
     * This is a recursive helper for {@link #AVLNode(Collection)}. A call to this method will
     * construct the subtree for elements {@code start} through {@code end} of the collection,
     * assuming the iterator {@code e} already points at element {@code start}.
     *
     * @param iterator
     *          an iterator over the collection, which should already point to the element at index
     *          {@code start} within the collection
     * @param start
     *          the index of the first element in the collection that should be in this subtree
     * @param end
     *          the index of the last element in the collection that should be in this subtree
     * @param absolutePositionOfParent
     *          absolute position of this node's parent, or 0 if this node is the root
     * @param prev
     *          the {@code AVLNode} corresponding to element (start - 1) of the collection, or null
     *          if start is 0
     * @param next
     *          the {@code AVLNode} corresponding to element (end + 1) of the collection, or null if
     *          end is the last element of the collection
     */
     ...
     *
     * @param index
     *          is the index of the position relative to the position of the parent node.
     * @param obj
     ...
     *
     * @param index
     *          is the index of the element to be removed relative to the position of the parent
    ...
    /**
     * Appends the elements of another tree list to this tree list by efficiently merging the two
     * AVL trees. This operation is destructive to both trees and runs in O(log(m + n)) time.
     *
     * @param otherTree
    ...
    public String toString() {
      return new StringBuilder().append("AVLNode(").append(relativePosition).append(',')
          .append(left != null).append(',').append(value).append(',')
          .append(getRightSubTree() != null).append(", faedelung ").append(rightIsNext).append(" )")
          .toString();
    }

    // Added equals to ensure evaluation correctness
    @Override
    public boolean equals(Object other) {
      return computeEquals(other, new HashSet<AVLNode>());
    }

    private boolean computeEquals(Object other, Set<AVLNode> visited) {
      AVLNode<E> o = (AVLNode<E>) other;
      if (o == null)
        return false;
      boolean sameValue = this.value == null && o.value == null
          || (this.value != null && this.value.equals(o.value));
      boolean sameBools = leftIsPrevious == o.leftIsPrevious && rightIsNext == o.rightIsNext;
      boolean sameInts = height == o.height && relativePosition == o.relativePosition;
      if (visited.add(this)) {
        if (sameValue && sameBools && sameInts) {
          // Reference eq
          boolean sameLeft = true;
          boolean sameRight = true;
          if (left != null)
            sameLeft = left.computeEquals(o.left, visited);
          else if (o.left != null)
            sameLeft = false;
          if (right != null)
            sameRight = right.computeEquals(o.right, visited);
