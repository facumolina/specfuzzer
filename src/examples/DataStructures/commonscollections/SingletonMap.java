 ...
 * <p>
 * The single key/value pair is specified at creation. The map is fixed size so any action that
 * would change the size is disallowed. However, the <code>put</code> or <code>setValue</code>
 * methods can <i>change</i> the value associated with the key.
 * </p>
 * <p>
 * If trying to remove or clear the map, an UnsupportedOperationException is thrown. If trying to
 * put a new mapping into the map, an IllegalArgumentException is thrown. The put method will only
 * succeed if the key specified is the same as the singleton key.
 * </p>
 * <p>
 ...
 * @since 3.1
 */
public class SingletonMap<K, V> implements KeyValue<K, V>, Serializable, Cloneable {
   ...
   *
   * @param key
   *          the key to set, must be the key of the map
   * @param value
   *          the value to set
  ...
  /**
   * Gets the unmodifiable keySet view of the map. Changes made to the view affect this map. To
   * simply iterate through the keys, use {@link #mapIterator()}.
   *
  ...
  /**
   * Gets the unmodifiable values view of the map. Changes made to the view affect this map. To
   * simply iterate through the values, use {@link #mapIterator()}.
   *
  ...
  /**
   * Values implementation for the SingletonMap. This class is needed as values is a view that must
   * update as the map updates.
   */
  ...
  @Override
  public String toString() {
    return new StringBuilder(128).append('{').append(getKey() == this ? "(this Map)" : getKey())
        .append('=').append(getValue() == this ? "(this Map)" : getValue()).append('}').toString();
  }
