...
MAJOR_HOME=../major/
sf110_project=$1
subject_sources=$SF110SRC/$sf110_project
target_file=$2
driver_base=$3
build_dir=$subject_sources/build/classes/
...
$MAJOR_HOME/bin/javac -cp $build_dir:$subject_sources/lib/* -nowarn -J-Dmajor.export.mutants=true -XMutator:ALL -d $build_dir $source_dir$target_file
...
cp_with_tests="$build_dir:$subject_sources/build/tests:$subject_sources/lib/*"
...
do
  ...
  javac -cp "$build_dir:$subject_sources/lib/*" -g $dir$target_file -d $build_dir
