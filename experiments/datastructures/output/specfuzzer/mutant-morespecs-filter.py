...
# This filter selects more than one spec from each bucket until all the fields are covered. 
fields_to_cover = {
    "List":{"x", "next", "SENTINEL"},
    "StackAr":{"topOfStack", "theArray"},
    "SortedList":{"header","size", "previous","next","element"},
    "SearchTree":{"root","size", "left","right","info"},
    "NodeCachingLinkedList":{"header"},
    "AvlTreeList":{"root"},
    "IntTreeSet":{"tree"}
}
...
def get_covering_fields(to_cover,spec):
    covering = set()
    for field in to_cover:
        if field in spec:
            covering.add(field)
    return covering

def cover_all_fields(specs_set):
    to_cover = fields_to_cover[clazz]
    covered = set()
    specs = set()
    for spec in specs_set:
        covered_fields = get_covering_fields(to_cover, spec)
        if (not covered_fields.issubset(covered)):
            specs.add(spec)
            covered = covered.union(covered_fields)
...
for vector, specs_set in buckets.items():
    ...
    print("Picked:")
    picked_set=set()
    picked_set = picked_set.union(cover_all_fields(sorted_set))
